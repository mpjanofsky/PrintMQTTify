...
- `docker-compose.yml`, `dockerfile`, `entrypoint.sh` — how the project runs containerized. Prefer these for reproducible local testing.
- `configs/cupsd.conf` — CUPS configuration; update here when adding printers or changing access rules.
...
- When changing Dockerfiles or `entrypoint.sh`, validate locally with `docker-compose up --build` and test printing with a local CUPS instance or by mocking `lp` (capture the command and file path).
